    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("EVET GÖRÜYORUM EFSANELER BURADA")
            ...
            # Welcome Devs
                ...
                update.effective_message.reply_text("EVET GÖRÜYORUM PRO OYUNCU BURADA!")

            # Welcome Sudos
            elif new_mem.id in SUDO_USERS:
                update.effective_message.reply_text("ayık ol kralim geldi 🤡👈")

            # Welcome Support
            elif new_mem.id in SUPPORT_USERS:
                update.effective_message.reply_text("Hey! bu benim geliştiricim hos geldin sahip")

            # Welcome Whitelisted
            elif new_mem.id in TIGER_USERS:
                update.effective_message.reply_text("Oof! Az önce @Drmehmetaktass ' karısı geldi ayık ol emrine amadeyim kralicem /promote !")

            # Welcome Tigers
            elif new_mem.id in WHITELIST_USERS:
                update.effective_message.reply_text("Oof! Az önce bir Kurt felaketi katıldı")

            # Welcome yourself
            elif new_mem.id == bot.id:
                update.effective_message.reply_text("merhaba 😎 beni kullandığın için teşekkürler, beni terfi ettirdiğinden emin ol, o zaman grubunu spam gönderenler için koruyabilirim 🤡🤡🤡👈")
            ...
            if should_mute:
                ...
                if welc_mutes == "strong":
                    ...
                    job_queue.run_once(
                        ...
                        ), 160, name="hoşgeldin sesini kapat"
                    )
...
def check_not_bot(member, chat_id, message_id, bot, job):
    ...
    if not member_status:
        ...
        try:
            bot.edit_message_text("*kullanıcıyı atar*\nHer zaman yeniden katılıp deneyebilirler.", chat_id=chat_id, message_id=message_id)
        except:
            pass
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text("ooo sed efsanesi gitti")

                return

            # Give the devs a special goodbye
            elif left_mem.id in DEV_USERS:
                update.effective_message.reply_text("Kahramanlar Birliği'nde sonra görüşürüz!")
                return
...
def welcome(bot: Bot, update: Update, args: List[str]):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("Tamam! Üye geldiğinde onları selamlayacağım")

        elif args[0].lower() in ("off", "no"):
            sql.set_welc_preference(str(chat.id), False)
            update.effective_message.reply_text("Aylak aylak dolaşacağım ve o zaman kimseyi hoş karşılamayacağım.")
...
def set_goodbye(bot: Bot, update: Update) -> str:
    ...
    if data_type is None:
        msg.reply_text("Ne cevap vereceğini belirtmedin!")
        return ""
...
def welcomemute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if len(args) >= 1:
        if args[0].lower() in ("off", "no"):
            ...
            msg.reply_text("Katılırken artık insanları sessize almayacağım!")
            return (f"<b>{html.escape(chat.title)}:</b>\n"
                    f"#WELCOME_MUTE\n"
                    f"<b>• Admin:</b> {mention_html(user.id, user.first_name)}\n"
...
def clean_welcome(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if clean_pref:
            update.effective_message.reply_text("İki güne kadar olan karşılama mesajlarını silmem gerekiyor.")
        else:
