...
def whois(bot: Bot, update: Update, args: List[str]):
    ...
    if user.id == OWNER_ID:
        text += "\n🚶🏻‍♂️Uff,Bu kişi benim Sahibim🤴\nOna karşı asla bir şey yapmam."
        
    elif user.id in DEV_USERS:
        text += "\n🚴‍♂️Pling,Bu kişi benim dev🤷‍♂️\nOna karşı asla bir şey yapmam!."
        
    elif user.id in SUDO_USERS:
        text += "\n🚴‍♂️Pling,Bu kişi benim sudo kullanıcılarımdan biri!" \
                    "Neredeyse sahibim kadar güçlü🕊 o yüzden izle.."
        
    elif user.id in SUPPORT_USERS:
        text += "\n🚴‍♂️Pling,Bu kişi benim destek kullanıcılarımdan biri! " \
                        "Tam bir sudo kullanıcısı değil, ama yine de sizi haritadan uzaklaştırabilir."
        
  
       
    elif user.id in WHITELIST_USERS:
        text += "\\n🚴‍♂️Pling,Bu kişi beyaz listeye alındı! " \
                        "Bu, onları yasaklamama/tekmelememe izin verilmediği anlamına geliyor."
